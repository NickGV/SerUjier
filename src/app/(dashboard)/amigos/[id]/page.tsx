...
import { getAmigoById, updateAmigo, deleteAmigo } from '@/shared/lib/utils';

interface AmigoDetail {
  id: string;
  nombre: string;
  telefono?: string;
  notas?: string;
...
const AmigoDetail = () => {
  ...
  const [amigo, setAmigo] = useState<AmigoDetail | null>(null);
  ...
  useEffect(() => {
    if (id) {
      const fetchAmigo = async () => {
        try {
          const data = await getAmigoById(id);
          setAmigo(data);
          setEditedData({
            nombre: data.nombre || '',
  ...
  const handleSave = async () => {
    ...
    try {
      await updateAmigo(amigo.id, editedData);
      setAmigo({ ...amigo, ...editedData });
      setIsEditing(false);
    } catch (err) {
    ...
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          ...
          <p className="text-gray-600">Cargando información del amigo...</p>
        </div>
      </div>
    );
  }
        ...
        <Card className="max-w-md w-full">
          <CardContent className="p-6 text-center">
            ...
            <p className="text-gray-600 mb-4">
              El amigo que buscas no existe o ha sido eliminado.
            </p>
            <Button onClick={() => router.back()} variant="outline">
              Volver
            </Button>
        ...
        <CardHeader className="px-3 sm:px-4 md:px-6 lg:px-8 py-4 sm:py-5 md:py-6">
          <div className="flex items-center gap-2 sm:gap-3 md:gap-4">
            ...
            <CardTitle className="text-lg sm:text-xl md:text-2xl lg:text-3xl font-bold text-gray-800 flex items-center gap-2 sm:gap-3 min-w-0">
              <div className="w-8 h-8 sm:w-10 sm:h-10 md:w-12 md:h-12 bg-gradient-to-r from-emerald-600 to-emerald-700 rounded-lg sm:rounded-xl flex items-center justify-center flex-shrink-0">
                <User className="w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6 text-white" />
              </div>
              <span className="truncate">Detalle del Amigo</span>
            </CardTitle>
          </div>
        </CardHeader>
      </Card>
        ...
        <CardContent className="p-4 sm:p-6 md:p-8 text-center">
          <div className="w-16 h-16 sm:w-20 sm:h-20 md:w-24 md:h-24 bg-white/20 rounded-xl sm:rounded-2xl flex items-center justify-center mx-auto mb-3 sm:mb-4">
            <span className="text-2xl sm:text-3xl md:text-4xl font-bold">
              {(isEditing ? editedData.nombre : amigo.nombre)
                .charAt(0)
                .toUpperCase()}
            </span>
          </div>
          ...
          <p className="text-emerald-200 text-sm sm:text-base md:text-lg px-2">
            {isEditing ? editedData.telefono : amigo.telefono || 'Sin teléfono'}
          </p>
        </CardContent>
      </Card>
            ...
            {!isEditing ? (
              ...
              <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 w-full sm:w-auto">
                ...
                <Button
                  size="lg"
                  onClick={handleSave}
                  className="bg-emerald-600 hover:bg-emerald-700 h-10 sm:h-12 px-4 sm:px-6 rounded-lg sm:rounded-xl font-semibold text-sm sm:text-base order-1 sm:order-2"
                >
          ...
          <div>
            ...
            {isEditing ? (
              <Input
                ...
                className="h-10 sm:h-12 text-sm sm:text-base rounded-lg sm:rounded-xl border-2 border-gray-200 focus:border-emerald-400"
              />
            ) : (
          ...
          <div>
            ...
            {isEditing ? (
              <Textarea
                ...
                className="rounded-lg sm:rounded-xl min-h-[100px] sm:min-h-[120px] text-sm sm:text-base border-2 border-gray-200 focus:border-emerald-400"
                placeholder="Agregar notas sobre el amigo..."
              />
            ) : (
              <p className="text-gray-800 bg-gray-50 p-3 sm:p-4 rounded-lg sm:rounded-xl min-h-[100px] sm:min-h-[120px] text-sm sm:text-base">
                {amigo.notas || 'Sin notas adicionales'}
              </p>
            )}
          </div>

          {/* Registration Date */}
          <div className="flex items-center gap-3 sm:gap-4 p-3 sm:p-4 bg-emerald-50 rounded-lg sm:rounded-xl">
            ...
            <div className="min-w-0">
              ...
              <p className="text-gray-600 text-sm sm:text-base">
                {new Date(amigo.fechaRegistro).toLocaleDateString('es-ES', {
                  year: 'numeric',
                  month: 'long',
                  day: 'numeric',
                })}
      ...
      <Card className="bg-white/90 backdrop-blur-sm border-0 shadow-lg">
        ...
        <CardContent className="px-3 sm:px-4 md:px-6 lg:px-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
            ...
            <div className="flex flex-col gap-2 sm:gap-3 p-3 sm:p-4 bg-emerald-50 rounded-lg sm:rounded-xl">
              <span className="text-sm sm:text-base font-semibold text-gray-700">
                Días desde registro:
              </span>
              <Badge
                variant="outline"
                className="bg-emerald-100 text-emerald-700 border-emerald-300 text-xs sm:text-sm md:text-base px-2 sm:px-3 md:px-4 py-1 sm:py-2 w-fit"
              >
                {Math.floor(
                  (new Date().getTime() -
                    new Date(amigo.fechaRegistro).getTime()) /
                    (1000 * 60 * 60 * 24)
                )}{' '}
                días
              </Badge>
      ...
      <div className="space-y-3 sm:space-y-4">
        {amigo.telefono && (
          <Button
            ...
            onClick={() => window.open(`tel:${amigo.telefono}`, '_self')}
          >
            ...
            <AlertDialogContent className="sm:max-w-md w-[95vw] sm:w-full mx-auto">
              <AlertDialogHeader className="text-center">
                ...
                <AlertDialogDescription className="text-sm sm:text-base text-gray-600 mt-2">
                  ...
                  la información de{' '}
                  <strong className="text-gray-800">{amigo.nombre}</strong> de
                  la base de datos.
                </AlertDialogDescription>
              </AlertDialogHeader>
