...
import { getVisitaById, updateVisita, deleteVisita } from '@/shared/firebase';
...
const VisitaDetail = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      await updateVisita(visita.id, editedData);
      setVisita({ ...visita, ...editedData });
      setIsEditing(false);
    } catch (err) {
    ...
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          ...
          <p className="text-gray-600">Cargando información de la visita...</p>
        </div>
      </div>
    );
  }
        ...
        <Card className="max-w-md w-full">
          <CardContent className="p-6 text-center">
            ...
            <p className="text-gray-600 mb-4">
              La visita que buscas no existe o ha sido eliminada.
            </p>
            <Button onClick={() => router.back()} variant="outline">
              Volver
            </Button>
        ...
        <CardHeader className="px-3 sm:px-4 md:px-6 lg:px-8 py-4 sm:py-5 md:py-6">
          <div className="flex items-center gap-2 sm:gap-3 md:gap-4">
            ...
            <CardTitle className="text-lg sm:text-xl md:text-2xl lg:text-3xl font-bold text-gray-800 flex items-center gap-2 sm:gap-3 min-w-0">
              ...
              <span className="truncate">Detalle de la Visita</span>
            </CardTitle>
          </div>
        </CardHeader>
      </Card>
        ...
        <CardContent className="p-4 sm:p-6 md:p-8 text-center">
          <div className="w-16 h-16 sm:w-20 sm:h-20 md:w-24 md:h-24 bg-white/20 rounded-xl sm:rounded-2xl flex items-center justify-center mx-auto mb-3 sm:mb-4">
            <span className="text-2xl sm:text-3xl md:text-4xl font-bold">
              {(isEditing ? editedData.nombre : visita.nombre)
                .charAt(0)
                .toUpperCase()}
            </span>
          </div>
          ...
          <div>
            ...
            {isEditing ? (
              ...
              <p className="text-gray-800 bg-gray-50 p-3 sm:p-4 rounded-lg sm:rounded-xl min-h-[100px] sm:min-h-[120px] text-sm sm:text-base">
                {visita.notas || 'Sin notas adicionales'}
              </p>
            )}
          </div>
            ...
            <div className="min-w-0">
              ...
              <p className="text-gray-600 text-sm sm:text-base">
                {visita.fechaRegistro
                  ? new Date(visita.fechaRegistro).toLocaleDateString('es-ES', {
                      year: 'numeric',
                      month: 'long',
                      day: 'numeric',
                    })
            ...
            <div className="flex flex-col gap-2 sm:gap-3 p-3 sm:p-4 bg-blue-50 rounded-lg sm:rounded-xl">
              ...
              >
                {visita.fechaRegistro
                  ? `${Math.floor(
                      (new Date().getTime() -
                        new Date(visita.fechaRegistro).getTime()) /
                        (1000 * 60 * 60 * 24)
                    )} días`
                  : 'Sin registro'}
              </Badge>
      ...
      <div className="space-y-3 sm:space-y-4">
        {visita.telefono && (
          <Button
            ...
            onClick={() => window.open(`tel:${visita.telefono}`, '_self')}
          >
            ...
            <AlertDialogContent className="sm:max-w-md w-[95vw] sm:w-full mx-auto">
              <AlertDialogHeader className="text-center">
                ...
                <AlertDialogDescription className="text-sm sm:text-base text-gray-600 mt-2">
                  ...
                  la información de{' '}
                  <strong className="text-gray-800">{visita.nombre}</strong> de
                  la base de datos.
                </AlertDialogDescription>
              </AlertDialogHeader>
