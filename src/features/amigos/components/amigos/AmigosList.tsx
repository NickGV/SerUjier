...
interface AmigosListProps {
  amigos: Amigo[];
  onEdit?: (amigo: Amigo) => void;
  onDelete?: (amigo: Amigo) => void;
  searchTerm: string;
}
...
export function AmigosList({
  ...
  const sortedAmigos = sortByNombre(amigos);

  return (
    <div className="space-y-3">
      {sortedAmigos.map((amigo) => (
        <Card
          key={amigo.id}
          className="bg-white/80 backdrop-blur-sm border-0 shadow-md cursor-pointer"
          onClick={() => router.push(`/amigos/${amigo.id}`)}
        >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-3 mt-2">
                    ...
                    <span className="text-xs text-gray-500">
                      Desde:{' '}
                      {amigo.fechaRegistro
                        ? new Date(amigo.fechaRegistro).toLocaleDateString(
                            'es-ES',
                            {
                              day: '2-digit',
                              month: '2-digit',
